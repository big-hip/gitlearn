...
文件已被纳入版本控制：如果在将文件添加到 .gitignore 之前，这些文件已经被 Git 跟踪（例如，已经使用 git add 命令添加到暂存区，或已经提交到本地仓库），那么 .gitignore 对它们将不起作用。对于这种情况，需要先从版本控制中移除这些文件，同时保留文件在磁盘上的物理存在，可以使用以下命令：
bash
